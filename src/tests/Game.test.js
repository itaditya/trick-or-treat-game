...
describe("Refresh feature", () => {
  it("should refresh after clicking the refresh button", () => {
    const { getByTestId } = render(<Game boardSizeX={2} boardSizeY={1} />);
    const board = getByTestId("game-table");
    ...
    const intialMoveCounter = getByTestId("moveCounter");
    const refreshBtn = getByTestId("refresh-btn");

    expect(Number(intialMoveCounter.innerHTML)).toBeGreaterThanOrEqual(0);
    expect(refreshBtn.innerHTML).toEqual("Refresh");
  });
});
